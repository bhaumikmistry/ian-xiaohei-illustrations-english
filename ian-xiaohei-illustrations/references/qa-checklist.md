...
- One image covers only one core structure.
...
- Realistic UI screenshot or tech-feel interface.
- Severe text errors or unreadable annotations.
- Image is too rigid — no absurd metaphor.
- Too similar to old example compositions in `assets/examples/`.
...
A high-quality image should make the reader first feel "that's a bit strange," then understand the structure within 1 second.

If the first impression is a tutorial page rather than an absurd product sketch on white paper, it doesn't pass.
